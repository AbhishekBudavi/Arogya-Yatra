...
const PrescriptionList = ({base,goBack}) => {
  ...
  const prescriptionIds = ['MED-10001', 'MED-10002']; // Add more as needed
  ...
  useEffect(() => {
    const fetchPrescriptions = async () => {
      ...
      setPrescriptions(allPrescriptions.filter(Boolean));
    };
    fetchPrescriptions();
  }, []);
    ...
    <div className="min-h-screen bg-gray-50 p-4 md:p-6 lg:p-8">
          ...
          <div key={prescription.id} className="bg-white rounded-2xl shadow-lg hover:shadow-xl transition-all duration-300 overflow-hidden border border-gray-100">
            <div className="p-6">
              <div className="flex justify-between items-start mb-4">
                <div>
                  <h3 className="font-bold text-xl text-gray-900 mb-1">{prescription.title}</h3>
                  <p className="text-sm text-gray-600 font-medium">Prescription</p>
                </div>
                <div className={`px-4 py-2 rounded-full text-sm font-semibold border flex items-center space-x-2 ${getStatusColor(prescription.status)}`}>
              ...
              <div className="bg-gradient-to-r from-blue-50 to-indigo-50 rounded-xl p-4">
                ...
                <p className="text-gray-700 leading-relaxed text-justify">
                  {expandedSummaries[prescription.id]
                    ...
                    : truncateText(prescription.aiSummary)}
                </p>
                {prescription.aiSummary.length > 120 && (
                  <button
      ...
      {filteredPrescriptions.length === 0 && (
        <div className="text-center py-16">
          ...
          <Link href={`${base}/prescriptions/prescription-form`} className="px-6 py-3 bg-gradient-to-r from-blue-600 to-indigo-600 text-white font-semibold rounded-xl shadow-lg hover:shadow-xl transform hover:scale-105 transition-all duration-200">
