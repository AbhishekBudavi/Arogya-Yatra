...
const Navbar = ({ collapsed, toggleSidebar,setSidebarOpen }) => {
    ...
    return (
        <nav className="bg-white/80 ml-5 backdrop-blur-xl border-b border-gray-100/50 rounded-3xl shadow-lg transition-all duration-300 sticky top-0 z-40">
      ...
      >
                        ...
                        <div className="hidden md:flex items-center space-x-2">
                            ...
                            <span className="text-gray-800 font-semibold">{patientData.name}</span>
                    ...
                    <div className="flex items-center space-x-4">
                        {/* Quick Stats */}
                        <div className="hidden lg:flex items-center space-x-4 px-4 py-2 bg-gradient-to-r from-blue-50 to-purple-50 rounded-2xl border border-blue-100/50">
                            <div className="flex items-center space-x-2">
                                <div className="w-2 h-2 bg-green-400 rounded-full animate-pulse"></div>
                                <span className="text-sm text-gray-600 font-medium">5 Online</span>
                            </div>
                            <div className="w-px h-4 bg-gray-300"></div>
                            <div className="flex items-center space-x-2">
                                <Calendar className="h-4 w-4 text-blue-500" />
                                <span className="text-sm text-gray-600 font-medium">12 Today</span>
                            </div>
                        </div>
                        ...
                        <div className="relative">
                            ...
                            >
                                ...
                                <div className="hidden md:flex flex-col items-start">
                                    <span className="text-sm font-semibold text-gray-800">{patientData.name}</span>
                                    <span className="text-xs text-gray-500">{patientData.role}</span>
                                </div>

                                <ChevronDown className={`h-4 w-4 text-gray-500 transition-transform duration-300 ${profileDropdown ? 'rotate-180' : ''}`} />
                            </button>
                                    ...
                                    <div className="px-4 py-3 border-b border-gray-100/50">
                                        <div className="flex items-center space-x-3">
                                            ...
                                            <div>
                                                ...
                                                <p className="text-sm text-gray-500">{patientData.role}</p>
                                                <p className="text-xs text-gray-400">Last visit: {patientData.lastVisit}</p>
                                            </div>
                                        </div>
                                    </div>
                                    <div className="py-2">
